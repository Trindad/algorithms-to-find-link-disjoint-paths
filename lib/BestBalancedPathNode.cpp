 ...
 vector< vector<int> > BestBalancedPathNode::compareWithOthers(Graph g,vector<int> &p1, vector<int> &p2) 
{
     ...
     */
    for ( int u = 1; u < (int)p1.size()-1; u++)
    {
        for (int v = 1; v < (int)p2.size()-1; v++)
        {
            if (find(p2.begin(),p2.end(),p1[u]) != p2.end())
            {
                // discardCommonEdge(p1,p2,p1[u],p2[v]);
 ...
 */
int BestBalancedPathNode::limitNumberOfNodesInPath(Graph g, int source,int target)
{
